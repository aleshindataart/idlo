...
class BlazyLayoutAdmin extends BlazyAdminBase implements BlazyLayoutAdminInterface {
  ...
  public function formStyles(array &$form, array $settings, array $excludes = []): void {
    ...
    foreach (Defaults::sublayoutSettings() as $key => $value) {
      ...
      if ($key == 'ete') {
        $title = 'Edge to edge';
        $description = $this->t('If enabled, will make the main background span edge to edge. Works better with <b>Max width</b> option, and themes with wide content region and without sidebars. Try Bartik if any issues.');
      }
      elseif ($key == 'padding') {
